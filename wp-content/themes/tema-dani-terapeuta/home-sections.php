<section id="section-1" class="home-section">
      <div class="container">
    <h1 class="text-white terapeuta">Danielle Zagonel Machado</h1>
    <h2 class="nome text-white">Terapeuta</h2>
    </div>
</section>

<section id="section-2" class="home-section">
    <div class="container">
   <div class="row justify-content-around">
    <div class="col-6 mt-5">
        <img src="http://localhost/dani-psico/wp-content/uploads/2023/01/foto-dani-circular.png" alt="foto Danielle Zagonel Machado" alt='foto Danielle Zagonel Machado'>
    </div>
    <div class="col-6 mt-5">
        <h3>Lorem ipsum dolor sit amet consectetur adipisicing elit. Ratione, quidem quod. Velit, delectus officia. Voluptate adipisci quaerat laboriosam voluptatem ea repudiandae aspernatur, qui vel doloribus at quisquam, iste assumenda quia!</h3>
    </div>
   </div>
</div>
</section>

<section id="section-3" class="home-section">
   <h1>TERAPIAS</h1>
</section>
<section id="section-4" class="home-section">
    <h1>FORMULÁRIO DE CONTATO</h1>
</section>